...
+ (void)verifyGetPhoneNumberWith:(NSDictionary *)result completion:(void (^) (NSError *error, NSString *phone))handler{
    ...
    if (!result) {
        NSError *error = [NSError errorWithDomain:@"com.fvd.error" code:0 userInfo:@{@"description":@"获取完整手机号参数异常"}];
        if (handler) {
            handler(error, nil);
        }
        return;
                ...
                if ([responseObject isKindOfClass:[NSDictionary class]] && [responseObject[@"status"] intValue] == 200)
                {
                    ...
                    if (![[NSThread currentThread] isMainThread]) {
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            ...
                            {
                                handler([NSError errorWithDomain:@"com.fvd.error" code:0 userInfo:@{@"description":@"数据错误"}], nil);
                            }
                        });
                    }
                    else
