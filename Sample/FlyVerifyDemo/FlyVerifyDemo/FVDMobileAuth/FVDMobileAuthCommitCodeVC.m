...
@interface FVDMobileAuthCommitCodeVC ()<UITextFieldDelegate>
{
...
//result
@property (nonatomic, copy) FVFlyVerifyCommitCodeResultHanler result;

@property (nonatomic, copy) NSDictionary *tokenInfo;

@end

@implementation FVDMobileAuthCommitCodeVC
...
-(void)loadCustomView
{
    self.view.backgroundColor = FVDDemoRGBA(74,159,163, 1);
    ...
    ({
        ...
        ({
            ...
            countryLabel.text = [NSString stringWithFormat:@"%@    %@",[FVDDemoHelper currentCountryName],[FVDDemoHelper currentZone]];
        ...
        _zone = [FVDDemoHelper currentZone];
        ...
        if([FVDDemoHelper currentCountryName] && [FVDDemoHelper currentZone])
        {
            ...
            if(!secdemo_zone && !secdemo_country)
            {
                ...
                [[NSUserDefaults standardUserDefaults] setObject:[FVDDemoHelper currentCountryName] forKey:@"secdemo_country"];
                [[NSUserDefaults standardUserDefaults] setObject:[FVDDemoHelper currentZone] forKey:@"secdemo_zone"];
...
-(void)loadCustomLayout
{
    ...
    // back button
    {
        NSLayoutConstraint *topConstraint = [NSLayoutConstraint constraintWithItem:self.backBtn attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:bgView attribute:NSLayoutAttributeTop multiplier:1.0 constant:(FVD_StatusBarSafeBottomMargin + 10)];
...
-(void)verify:(UIButton*)btn
{
    ...
    [FVSDKHyVerify mobileAuthWith:_phone token:self.tokenInfo timeOut:5 completion:^(NSDictionary * _Nullable result, NSError * _Nullable error) {
